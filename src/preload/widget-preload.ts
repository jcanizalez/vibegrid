...
const widgetApi = {
  onStatusUpdate: (callback: (agents: WidgetAgentInfo[]) => void) => {
    const listener = (_: Electron.IpcRendererEvent, agents: WidgetAgentInfo[]): void =>
      callback(agents)
    ipcRenderer.on(IPC.WIDGET_STATUS_UPDATE, listener)
    return () => {
      ipcRenderer.removeListener(IPC.WIDGET_STATUS_UPDATE, listener)
    }
  },

  onPermissionRequest: (callback: (request: PermissionRequestInfo) => void) => {
    const listener = (_: Electron.IpcRendererEvent, request: PermissionRequestInfo): void =>
      callback(request)
    ipcRenderer.on(IPC.WIDGET_PERMISSION_REQUEST, listener)
    return () => {
      ipcRenderer.removeListener(IPC.WIDGET_PERMISSION_REQUEST, listener)
    }
  },
  ...
  onPermissionCancelled: (callback: (requestId: string) => void) => {
    ...
    return () => {
      ipcRenderer.removeListener(IPC.WIDGET_PERMISSION_CANCELLED, listener)
    }
  },
  ...
  focusTerminal: (id: string) => ipcRenderer.send(IPC.WIDGET_FOCUS_TERMINAL, id),

  hideWidget: () => ipcRenderer.send(IPC.WIDGET_HIDE),

  showApp: () => ipcRenderer.send('widget:show-app'),

  setViewMode: (mode: 'full' | 'compact') => ipcRenderer.send('widget:set-view-mode', mode)
}
